...
document.getElementById('product-button').addEventListener('click', e => openInfo(e, 'produits'));
document.getElementById('cart-button').addEventListener('click', e => openInfo(e, 'panier'));


// Extrait en partie de https://www.w3schools.com/howto/howto_js_tabs.asp
...
// generate a checkbox list from a list of produits
...
function populateListProductChoices() {
    ...
    if (restrictions === '') {
        preselectedItems.innerHTML = 'Tous les produits que nous avons.'
    } else {
        preselectedItems.innerHTML = 'Produits selectionn&eacute;s: ' + restrictions
    }

    // s2 represents the <div> in the produits tab, which shows the product list, so we first set it empty
    s2.innerHTML = '';

    // obtain a reduced list of produits based on restrictions
    const optionArray = restrictListProducts(produits, checkboxes);
...
function selectedItems() {
    ...
    const panier = document.getElementById('displayCart');
    panier.innerHTML = '';
    ...
    panier.appendChild(document.createTextNode('Total price is $' + totalPrice));
